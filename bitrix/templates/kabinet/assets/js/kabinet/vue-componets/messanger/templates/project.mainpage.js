...
<div :id="$id('messangerBlock')" class="messanger-block mt-5" v-if="typeof datamessage[projectID] != 'undefined'">  
    <div ref="messagelist" class="messange-list p-2 messanger-main-page" v-if="datamessage[projectID].length>0">
        ...
        <div ref="showmoreblock" class="mess p-2 mb-4" v-if="datamessage[projectID].length>4">
            <button type="button" class="btn btn-sm btn-link" @click="showMore()">Показать еще <i class="fa fa-refresh" aria-hidden="true"></i></button>
        </div>
    
		<div v-for="mess_item in datamessage[projectID]">
			...
			<div class="row" v-if="mess_item.UF_TYPE == 3">
				...
				<div class="col-10 text-block-mess">
					<div class="d-flex flex-wrap align-items-end header-message-block">
						<div class="user-title mb-3">{{mess_item.UF_AUTHOR_ID_ORIGINAL.PRINT_NAME}}</div>
					...
					<div v-html="mess_item.UF_MESSAGE_TEXT_ORIGINAL" class="mb-5 message_text_orgn"></div>
				</div>
			</div>
