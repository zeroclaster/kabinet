...
EOT;
		...
		elseif($arResult['userField']['SETTINGS']['DISPLAY'] === EnumType::DISPLAY_CHECKBOX)
		{
				...
				>
					<label>
						<input
							...
							tabindex="0"
                            v-model="<?=$arResult["additionalParameters"]["attribute"]["v-model"]?>"
						>
						<?= $val ?>
					</label>
					<br>
			...
			$fieldlist = $GLOBALS["USER_FIELD_MANAGER"]->getUserFields($arResult['userField']["ENTITY_ID"],null,LANGUAGE_ID);
			$nameField = $arResult['userField']["FIELD_NAME"].'_USER';

			//\Dbg::var_dump($arResult);

            if (isset($fieldlist[$nameField])) {?>
                <customoption v-model="<?=$arResult["additionalParameters"]["VMODEFIELD"]?>.<?=$nameField?>"/>
            <?}
